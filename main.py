...
from kivymd.uix.button import MDRoundFlatIconButton, MDIconButton, MDFlatButton, MDFloatingActionButton, MDRectangleFlatButton, MDFillRoundFlatButton
...
<MyPicture>:
    ...
    MDIconButton:
        ...
        on_release: app.openv(root.image_id)
<URL>:
    size_hint_y: None
    ...
    MDIconButton:
        ...
        icon_color: app.theme_cls.primary_color
        pos_hint: {'right': 1}
        on_release: app.openURL(root.http)
...
<MyDialog>:
    MDBoxLayout:
        ...
        MDFloatLayout:
            ...
            size_hint_x: .4 if app.platform != "android" else 1
            size_hint_y: .4 if app.platform == "android" else 1
            pos: (0, 0)
            radius: (5, 0, 0, 5)
            MDIcon:
                icon: root.source
...
<Onboarding>:
    ...
    on_enter: setattr(boarding.ids.rounded_box.children[0], 'text', 'Завершить')
        ...
        AKOnboarding:
            ...
            MyAKOnboardingItem:
                ...
                text:
                    ...
                    " и включающий в себя структуру, " \
                    "предметное содержание и метаданные о них."
                title: "Электронный образовательный ресурс"
            ...
            MyAKOnboardingItem:
                source: "youwilllearn.png"
                text:
                    '''Основные команды SQLite
                    Как создавать свои небольшие БД и управлять ими
                    Как просматривать другие БД, загружая их
                    Создание таблиц, триггеров, представлений и т.д.'''
                title: "Вы изучите"
        ...
        MDBoxLayout:
            ...
            MDBoxLayout:
                ...
                MDIconButton:
                    ...
                    on_release: app.exportDB(path.text)
                    text: " "
                    height: path.height
                    theme_icon_color: "Custom"
                    size_hint_x: .1
...
class MyApp(MDApp):                                                                                 #Класс главного окна
    def __init__(self):
        super().__init__()
        self.title = 'Электронный образовательный ресурс "SQLite для начинающих"'                                           #Изменение заголовка окна
        self.icon = 'database.png'                                                                  #Изменение иконки окна
        self.theme_cls.theme_style = 'Light'
        self.theme_cls.primary_palette = 'Blue'
        self.theme_cls.theme_style_switch_animation = True
        self.platform = platform
        self.dir = os.path.abspath(os.getcwd()).replace('\\', '/') + '/'
    ...
    def cards(self):                                                                                #Функция, которая вызывается в случае, когда окно приветствия пройдено
        ...
        for key in global_data:
            if key != 'theme' and key != 'obuch_passed' and key != 'databases':
                ...
                self.text = list(global_data[key].keys())   #список уроков раздела key
                if self.text[-1] == 'Тест':
    ...
    def openv(self, pos):
        self.viewer.open()
        self.viewer.carousel.load_slide(self.viewer.carousel.slides[pos])
    ...
    def panel_open(self, item):                                                                     #Функция, которая вызывается на открытие раздела, открывает список уроков
        ...
        for key in global_data:
            if key != 'theme' and key != 'obuch_passed' and key != 'databases':
                if item.text in list(global_data[key].keys()):
                    ...
                    lessons = list(global_data[key].keys())
                    j = lessons.index(item.text)
                    if lessons[-1] == 'Тест' and j == len(lessons) - 2:
                        self.test = True
                    else:
                        self.test = False
                    break
        ...
        image_id = 0
        self.control = False #есть ли задание в конце урока
        try:
            ...
            for elem in root.iter():
                if elem.tag == 'text':
                    ...
                    imgbox = MyPicture(image = fr'{k + 1}/{elem.text}', image_id = image_id)
                    self.viewer.add_widget(AKImageViewerItem(source = fr'{k + 1}/{elem.text}'))
                    ...
                    url = URL(image = fr'{k + 1}\URL.jpeg', http = elem.text, image_id = image_id)
                    self.viewer.add_widget(AKImageViewerItem(source = fr'{k + 1}\URL.jpeg'))
                    box.add_widget(url)
                    image_id += 1
                elif elem.tag == 'code':
                    ...
                    if attrib == None:
                        ...
                        self.control = True #есть задание в конце урока
                        inp = TextInp(text = elem.text, md_bg_color = [245,245,245,1], specific_text_color=myapp.theme_cls.text_color)
                        inp.isKontrol = True
                        inp.isReadOnly = False
        ...
        if self.test:
            box.add_widget(MDRectangleFlatButton(text = 'Пройти тест', on_release = self.open_test, size_hint = (1, None)))
        if not self.control: #если нет задание в конце урока - он засчитывается просто за пройденный
            if not global_data[self.razdel][self.lesson]: #если урок еще не пройден
                global_data[self.razdel][self.lesson] = True
                insertToJSON()
                self.cur_card.value += 100/len(global_data[self.razdel].keys())            
    ...
    def openURL(self, http):
        try:
            webbrowser.open(http)
        except Exception as e:
            toast(e)

    def exportDB(self, DBName): #функция экспорта БД
        def select_path(path):
            self.file_manager.close()
            if path == '':
                path = myapp.dir
            path = path.replace('\\', '/')
            dial = MyDialog(textBox = 'Назовите ваш файл:', source = 'database-export', color = myapp.theme_cls.primary_color)
            dial.ids.box.add_widget(MDTextField(hint_text="Путь", mode="rectangle", text = path, readonly=True))
            dial.ids.box.add_widget(MDTextField(hint_text="Имя файла", mode="rectangle"))
            dial.ids.box.add_widget(MDRectangleFlatButton(text = 'Экспортировать БД', on_release = lambda x: dial.exportDB(x, DBName)))
            dial.open()
        ...
        self.file_manager = MDFileManager(exit_manager=exit_manager, select_path = select_path, show_hidden_files = False, selector='folder')
        if (platform == 'android'):
          try:
...
class ScreenTest(MDScreen):
    ...
    def createTest(self):
        ...
        try:
            ...
            self.board.ids.rounded_box.children[0].text = 'Завершить'
            ...
            for elem in root.iter('question'):
                ...
                widget = MDBoxLayout(orientation = 'vertical', padding=10, spacing = 20, adaptive_height=True, size_hint_y=None)
...
class ScreenMyDB(MDScreen):
    ...
    def tabSwitch(self, instance_tabs, instance_tab, instance_tab_label, tab_text):
        ...
        connection.execute('PRAGMA foreign_keys=on;') #Соединение с базой данных
        cursor = connection.cursor()
        cursor.execute('select type, name from sqlite_master;')                          #Команда для вывода всех таблиц из этой базы
        ...
        for item in result:
            ...
            widget.add_widget(MDFillRoundFlatButton(text = item[1], on_release = self.openDBObject, font_style='Subtitle1'))
        if len(self.ids.DBTriggers.children) == 0:
            self.ids.DBTriggers.add_widget(MDFillRoundFlatButton(text = 'Здесь пока пусто', font_style='Subtitle1'))
        if len(self.ids.DBTables.children) == 0:
            self.ids.DBTables.add_widget(MDFillRoundFlatButton(text = 'Здесь пока пусто', font_style='Subtitle1'))
        if len(self.ids.DBViews.children) == 0:
            self.ids.DBViews.add_widget(MDFillRoundFlatButton(text = 'Здесь пока пусто', font_style='Subtitle1'))
        if len(self.ids.DBIndex.children) == 0:
            self.ids.DBIndex.add_widget(MDFillRoundFlatButton(text = 'Здесь пока пусто', font_style='Subtitle1'))
        if path == myapp.dir + 'main.db':
    ...
    def openDBObject(self, chip):
        ...
        if chip.parent==self.ids.DBTables:
            ...
            box.add_widget(MDDataTable(size_hint = (1, None), column_data = (('NAME', dp(30)), ('TYPE', dp(30)),
                                                                             ...
                                                                             ('PRIMARY KEY', dp(30))), row_data = cols, use_pagination=True, height=Window.height/2))
            ...
            for k in cols:
                k.append(dp(30))
            box.add_widget(MDDataTable(size_hint = (1, None), column_data = cols, row_data = data, use_pagination=True, height=Window.height/2))
    ...
    def on_leave(self):
        ...
        connection.execute('PRAGMA foreign_keys=on;') #Соединение с базой данных
        for row in global_data['databases'][1:]: #первую базу не берем, к ней подключились
            name = row.split('/')[-1].replace('.db', '')
            connection.execute('ATTACH DATABASE "' + row + '" AS "' + name + '"')
        cursor = connection.cursor()
...
class MyDialog(MDDialog):                                                                             #Класс информационного окна
    ...
    def exportDB(self, button, DBName):
        path = button.parent.children[2].text
        if path != '':
            path += '/'
        else:
        ...
        conn2.close()
        self.dismiss()
        MyDialog(textBox = 'Ваша БД успешно экспортирована в sql-файл', source = 'check-circle', color = myapp.theme_cls.primary_color).open()
...
class Terminal(MDBoxLayout):                                                                        #Класс терминала
    ...
    def check(self, textinp, obuch):                                                           #Функция проверки введенного кода
        global global_data, cursor, connection
        try:
            if isinstance(self.children[0], MDDataTable): #очистка от предыдущей команды
                self.remove_widget(self.children[0])
            c = textinp.text.replace('\n', ' ').strip()
            c_casefold = c.casefold()
            if c_casefold.count('attach'):
                pass
            if c_casefold.startswith('.'):
                if c_casefold.count('.schema'):
                    table_name = c.split(' ')[1].replace(';', '')
                    if table_name in [i[0] for i in cursor.execute('select name from sqlite_master;').fetchall()]:
                        ...
                        for i in cols:
                            ...
                            if i[4] == 1:
                                ...
                                i[4] = ("close-circle", 'red', '')
                        self.add_widget(MDDataTable(size_hint = (1, None), column_data = (('NAME', dp(30)), ('TYPE', dp(30)),
                                                                                          ('NOT NULL', dp(30)), ('DEFAULT', dp(30)),
                                                                                          ('PRIMARY KEY', dp(30))), row_data = cols, use_pagination=True, height = Window.height/2))
                    else:
                        toast('Такой таблицы нет, создайте ее')
                elif c_casefold.count('.open'):
                    table_name = c.split(' ')[1].replace(';', '')
                    conn2 = sqlite3.connect(table_name)
                    for id_, name, filename in conn2.execute('PRAGMA database_list'):
                        if name == 'main' and filename is not None:
                            path = filename.replace('\\', '/')
                            break
                    conn2.close()
                    filename = path.split('/')[-1].replace('.db', '')
                    connection.execute('ATTACH DATABASE "' + path + '" as "' + filename + '"')
                    if path not in global_data['databases']:
                        global_data['databases'].append(path)
                        insertToJSON()
                        myapp.screenManager.get_screen('ScreenCards').ids.mydb.right_text = str(len(global_data['databases']))  
                    toast('Вы подключились к БД ' + table_name)
                elif c_casefold.count('.databases'):
                    cursor.execute("PRAGMA database_list;")
                    data = cursor.fetchall()
                    self.add_widget(MDDataTable(size_hint = (1, None), column_data = (('№', dp(10)), ('Псевдоним', dp(30)),
                                                                                          ('Путь', dp(100))), row_data = data,
                                                    use_pagination=True, height = Window.height/2))
                elif c_casefold.count('.tables'):
                    cursor.execute('select * from sqlite_master where type="table";')
                    res = cursor.fetchall()
                    cols = []
                    for k in cursor.description:
                        cols.append([k[0], dp(30)])
                    cols[-1][-1] = dp(100)
                    table = MDDataTable(size_hint = (1, None), column_data = cols, row_data = res, height = Window.height/2)
                    self.add_widget(table)
                elif c_casefold.count('.dump'):
                    table_name = c.split(' ')[1].replace(';', '')
                    myapp.exportDB(table_name)
                return
            try:
                table_name = Parser(c).tables[0]
            except:
                if (c_casefold.count(' temp ') or c_casefold.count(' temporary ')) and c_casefold.count('table'): #временная таблица
                    table_name = re.split(' table ', c, flags=re.IGNORECASE)[1].split(' ')[0].strip()
                elif c_casefold.count('view'): #представление
                    table_name = re.split(' view ', c, flags=re.IGNORECASE)[1].split(' ')[0].strip()
                elif c_casefold.count('trigger'): #триггер
                    table_name = re.split(' trigger ', c, flags=re.IGNORECASE)[1].split(' ')[0].strip()
                elif c_casefold.count('index'): #индекс
                    table_name = re.split(' index ', c, flags=re.IGNORECASE)[1].split(' ')[0].strip()
            if table_name[-1] == '(':
            ...
            cursor.execute(c)
            connection.commit()
            if c_casefold.count('create') == 1 and c_casefold.count('trigger'):
                toast('Триггер ' + table_name + ' был создан')
            elif c_casefold.count('create') and c_casefold.count('index'):
                toast('Индекс ' + table_name + ' был создан')
            elif c_casefold.count('create') and c_casefold.count('view'):
                toast('Представление ' + table_name + ' было создано')
                select = True
            elif c_casefold.count('create') and c_casefold.count('table'):
                cursor.execute('pragma table_info("' + table_name + '");')
                cols = [list(i[1:]) for i in cursor.fetchall()] #считывание столбцов
                for i in cols:
                    if i[2] == 1:
                ...
                self.add_widget(MDDataTable(size_hint = (1, None), column_data = (('NAME', dp(30)), ('TYPE', dp(30)),
                                                                                  ('NOT NULL', dp(30)), ('DEFAULT', dp(30)),
                                                                                  ('PRIMARY KEY', dp(30))), row_data = cols,
                                            use_pagination=True, height = Window.height/2))
                toast('Таблица ' + table_name + ' была создана')
            elif c_casefold.count('drop') == 1 and c_casefold.count('table'):
                toast('Таблица ' + table_name + ' была удалена')
            elif c_casefold.count('drop') == 1 and c_casefold.count('trigger'):
                toast('Триггер ' + table_name + ' был удален')
            elif c_casefold.count('drop') == 1 and c_casefold.count('view'):
                toast('Представление ' + table_name + ' было удалено')
            elif c_casefold.count('drop') == 1 and c_casefold.count('index'):
                toast('Индекс ' + table_name + ' был удален')
                ...
                select = True
            elif c_casefold.count('select'): #select
                data = cursor.fetchall()
                data_col = []
                for k in cursor.description:
                    data_col.append(((k[0], dp(30))))
                self.add_widget(MDDataTable(size_hint = (1, None), column_data = data_col, row_data = data,
                                            use_pagination=True, height = Window.height/2))
            if obuch == 'kontrol':
            ...
            if select:
                ...
                for k in cols:
                    k.append(dp(30))
                self.add_widget(MDDataTable(size_hint = (1, None), column_data = cols, row_data = data, use_pagination=True, height=Window.height/2))
        except Exception as e:
            if str(e).endswith('already exists'):
            ...
            elif str(e).count('has no column named') or str(e).startswith('no such column:'):
                toast('Не найден столбец ' + str(e).split()[-1])
            elif str(e).startswith('no such'):
                toast('Такого объекта БД нет, создайте его')
                ...
                toast('Нарушение целостноти внешнего ключа')
            elif str(e).startswith('Not supported query type'):
                pass #ошибка от Parser, не обрабатывает триггеры, представления, индексы и временные таблицы
            elif str(e).startswith('RIGHT and FULL OUTER JOINs are not currently supported'):
                MyDialog(textBox = 'RIGHT и FULL OUTER JOIN еще не поддерживаются', source='close-circle', color='red')
            else:
...
class URL(MDBoxLayout):
    image = StringProperty()
    http = StringProperty()
    image_id = NumericProperty()

class MyPicture(MDBoxLayout):
    image = StringProperty()
    image_id = NumericProperty()
...
if __name__ == '__main__':
    ...
    connection = sqlite3.connect('main.db')                                                           #Соединение с базой данных
